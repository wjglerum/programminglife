...
public class Patients extends Controller {
	...
	public static Result insert() throws SQLException {
		...
		if (addForm.hasErrors()) {
			...
			Patient p = patientService.add(Authentication.getUser().id, name, surname,
					fileName, fileSize);
			...
			for (Mutation m : mutations) {
				counter++;
				query += "(nextval('m_id_seq'::regclass)," + p.getId() + ",'"
						+ m.getMutationType() + "','" + m.getRsID() + "',"
						+ m.getChromosome() + ",'" + m.toAllelesString() + "',"
						+ m.getStart() + "," + m.getEnd() + "),";
				...
				if (counter % 1000 == 0) {
					...
					Logger.info("We hebben " + counter
							+ " mutations aan de database toegevoegd.");
			...
			query += ";";
			Database.insert("data", query);
			Logger.info("Er zijn in totaal " + counter
					+ " mutations aan de database toegevoegd.");
			// Make user happy
